...
class pre_bot(discord.Client):
    ...
    async def on_ready(self):
        await tree.sync(guild=discord.Object(id=1121639089897230516))
        self.synced = True
        print("Bot Online")
...
async def self(interaction: discord.Interaction, url:str):
    ...
    requests.get("https://api.trace.moe/search?anilistInfo&url={}".format(urllib.parse.quote_plus(url))).json() 
